...
export default async function login (): Promise<Client | FakeClient> {
...
async function getChannels (client: Client | FakeClient, channels: string[]): Promise<Array<TextChannel | DMChannel>> {
  return (
    await Promise.all(channels.map(id => getChannel(client, id)))
  ).filter(Boolean) as Array<TextChannel | DMChannel>
}
