  ...
  return new Promise(async (resolve, reject) => {
    ...
    try {
      ...
      if (err.code && err.code === 'ENOENT') {
        try {
          await util.promisify(require('mkdirp'))(configFolder)
          log.warn('getConfig', (path.join(configFolder, 'config.json') + ' not found, creating example config'))
          fs.copyFileSync(path.join(__dirname, '..', '..', 'config.example.json'), path.join(configFolder, 'config.json'))
        } catch (error) {
