...
module.exports = async (thread, clean = true) => {
  ...
  let name
  try {
    name = thread.isGroup ? (thread.name || thread.id) : ((thread.nicknames ? thread.nicknames[thread.id] : null) || (await getSender(thread.id)).name)
  } catch (err) {
  ...
  const cleanname = emojiStrip(removeAccents(name.toString())).trim().replace(/ /g, '-').replace(/\W-/g, '').replace(/(?![a-zA-Z0-9\-_])/g, '').toLowerCase()
