...
module.exports = async message => {
  ...
  var channel = await config.channels.getChannel(message.threadID, thread.name, isLinked)
  if (channel) channel.send(`*${isLinked ? getChannelName(thread, sender, message, false) + ': ' : ''}${message.logMessageBody}*`)
}
