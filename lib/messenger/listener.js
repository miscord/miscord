...
module.exports = async message => {
  ...
  await Promise.all(channels.map(async channel => {
    ...
    const files = m[1].files ? m[1].files.filter(f => f) : []
    if (m[1].files && m[1].files.length > files.length && config.messenger.attachmentTooLargeError) {
      messenger.client.sendMessage(Number(thread.id), `Your attachment couldn't be sent on Discord due to file size limits.`)
    }
    m[1].files = files
    if (!m[0] && !m[1].files.length) return log.debug('Not sending message, empty.')
