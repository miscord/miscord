...
module.exports = async (error, message) => {
  ...
  if (messenger.alreadyHandled.length > 100) messenger.alreadyHandled.splice(100)
  ...
  // get channel
  const channel = await channels.getChannel(message.threadID, cleanname)
  if (!channel) return
  log.verbose('messengerListener', 'Got Discord channel')

  const link = channels.getThreadIDs(channel.id)

  // find / create a webhook
  var webhook = discord.webhooks.find(webhook => webhook.channelID === channel.id)
  if (!webhook) {
  ...
  if (link.length > 1) {
    ...
    link.filter(id => id !== message.threadID).forEach(threadID => messenger.client.sendMessage(msg, threadID, sentMessageCallback))
  }
}
