...
module.exports = async message => {
  ...
  await Promise.all(channels.map(async channel => {
    ...
    if (!m[0] && !(m[1].files && m[1].files.length)) {
      if (config.messenger.attachmentTooLargeError) messenger.client.sendMessage(Number(thread.id), `Your message couldn't be sent on Discord due to file size limits.`)
