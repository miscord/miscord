...
module.exports = async message => {
  ...
  const { body: mBody, attachments } = await createMessage.messenger(thread, sender, message, thread.cleanName)
  log.debug('Created Messenger message')
  Promise.all(threads.map(async _thread => {
    if (mBody && mBody.trim()) {
      log.debug('Sending Messenger message')
      const info = await messenger.client.sendMessage(Number(_thread.id), mBody.toString())
