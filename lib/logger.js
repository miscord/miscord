  ...
  inspect (object, depth = 2, prefixLength = 20) {
    return util.inspect(
      object,
      {
        depth,
        colors: true,
        breakLength: (process.stdout.columns - (prefixLength + this.getScopeForLevel('info').length)) || undefined
      }
    )
  }
...
module.exports.inject = (configPath = getConfigDir()) => {
  ...
  const logStream = fs.createWriteStream(path.join(dir, filename))
  ...
  function bindWrite (write, stream) {
    return (string, encoding, fd) => {
      if (string.startsWith('close')) {
        const match = string.match(/close (\d+)/)
        if (match) {
