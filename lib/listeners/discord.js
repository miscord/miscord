...
module.exports = opts => {
  try {
    ...
    if (!config.messenger.separateImages) {
      config.messenger.client.sendMessage(msg, message.channel.topic, sentMessageCallback)
    } else {
      config.messenger.client.sendMessage(msg[0], message.channel.topic, sentMessageCallback)
      config.messenger.client.sendMessage(msg[1], message.channel.topic, sentMessageCallback)
    }
  } catch (e) {
    sendError(e)
  }
