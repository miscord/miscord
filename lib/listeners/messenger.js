...
module.exports = async (error, message) => {
  ...
  if (link) {
    ...
    let msg = {
      ...
      attachment: await Promise.all(message.attachments.map(getProxyURL).filter(el => el).map(getStreamFromURL))
    }
  ...
  getChannel({
    ...
    webhook.send(...m).then(message => { log.verbose('messengerListener', 'Sent message on Discord'); log.silly('messengerListener: sent message', message) })
  })
}
...
function getProxyURL (attachment) {
  var baseURL = attachment.image || attachment.url
  baseURL = baseURL ? url.parse(baseURL, true).pathname === '/safe_image.php' ? url.parse(baseURL, true).query.url : baseURL : undefined
  var path = url.parse(baseURL, true).pathname.toLowerCase()
  var proxyUrl = path.match(/(\.png|\.jpg|\.gif)$/i)
    ? 'https://images.weserv.nl/?url=' +
      encodeURIComponent(encodeURIComponent(baseURL.replace('https://', 'ssl:').replace('http://', '')))
    : baseURL
  log.info('messengerListener: proxyUrl', proxyUrl)
  return proxyUrl
}
