...
module.exports = async opts => {
  ...
  getChannel({
    ...
    if (
      !config.discord.noEmbeds &&                                  // embeds are enabled
      m.embed &&                                                   // there's an embed in current message
      lastMessage &&                                               // there is a "last message"
      lastMessage.embeds[0] &&                                     // the last message had embed
      lastMessage.embeds[0].author &&                              // that embed had author
      lastMessage.embeds[0].author.name === m.embed.author.name && // last message's author is the same as current's
      !lastMessage.embeds[0].image &&                              // there was no picture in the embed
      lastMessage.attachments.length === 0 &&                      // there were no attachments
      opts.message.attachments.length === 0 &&                     // current message also has no attachmtents
      m.length === 1                                               // there are no message options
    ) {
