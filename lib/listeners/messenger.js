...
module.exports = async opts => {
  try {
    ...
    let link = Object.entries(config.messenger.link).find(entry => entry.includes(message.threadID))
    if (link) {
      ...
      config.messenger.client.sendMessage(
        { body:
          createFBMessage(config, authorName, message.body, `Messenger: ${getChannelName(thread, sender, message, false)}`)
        },
        link.find(value => value !== message.threadID)
      )
    }
