...
module.exports = opts => {
  ...
  config.facebook.client.getThreadInfoGraphQL(message.threadID, (err, thread) => {
    ...
    config.facebook.client.getUserInfo(message.senderID, (err, sender) => {
      ...
      getChannel({
        ...
        if (!channel.lastMessageID) return config.discord.noEmbeds ? channel.send(m[0], m[1]) : channel.send(m)
        ...
        if (
          ...
          config.discord.noEmbeds ? channel.send(m[0], m[1]) : channel.send(m)
        }
      }).catch(sendError)
    })
  })
