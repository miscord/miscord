...
module.exports = config => {
  ...
  return discord.login(config.discord.token).then(async u => {
    ...
    if (config.custom && Object.keys(config.custom).length) Object.entries(config.custom).forEach((threadID, channelName) => config.discord.channels.set(threadID, category.children.find(el => el.topic === threadID && (el.name === channelName || el.id === channelName))))

    log.silly('login', 'Discord channels:', config.discord.channels)
