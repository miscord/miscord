...
It's not an error... unless you added the bot to your guild already.`)
    ...
    if (config.ownerID) {
      config.errorChannel = config.ownerID
      config.commandChannel = config.ownerID
    }

    if (config.errorChannel) {
      if (discord.users.has(config.errorChannel)) {
        config.errorChannel = await discord.users.get(config.errorChannel).createDM()
      } else if (discord.channels.has(config.errorChannel)) {
        config.errorChannel = discord.channels.get(config.errorChannel)
      } else {
    ...
    if (config.commandChannel) {
      if (discord.users.has(config.commandChannel)) {
        config.commandChannel = await discord.users.get(config.commandChannel).createDM()
      } else if (discord.channels.has(config.commandChannel)) {
        config.commandChannel = discord.channels.get(config.commandChannel)
      } else {
