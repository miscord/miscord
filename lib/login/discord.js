...
module.exports = config => {
  ...
  return discord.login(config.discord.token).then(async u => {
    ...
    config.discord.guild = config.discord.guild ? discord.guilds.find('name', config.discord.guild) : discord.guilds.first()
    ...
    Object.entries(config.messenger.link).forEach(entry => {
      config.custom[entry[1]] = config.custom[entry[0]]
    })
