...
module.exports = config => {
  ...
  return discord.login(config.discord.token).then(async u => {
    ...
    if (config.custom && Object.keys(config.custom).length) Object.entries(config.custom).forEach(entry => config.discord.channels.set(entry[0], category.children.find(el => el.name === entry[1] || el.id === entry[1])))
