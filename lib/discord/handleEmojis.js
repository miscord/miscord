...
module.exports = message => {
  ...
  for (let match of matches) {
    const emoji = discord.emojis.find(emoji => emoji.name.toLowerCase() === match.replace(/:/g, '').toLowerCase()) ||
    discord.guilds.GetAll('emojis').find(emoji => emoji.name.toLowerCase() === match.replace(/:/g, '').toLowerCase())
