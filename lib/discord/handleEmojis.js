...
module.exports = message => {
  ...
  for (let match of matches) {
    // discord.emojis.find(emoji => emoji.name.toLowerCase() === match.replace(/:/g, '').toLowerCase())
    const emoji = discord.guilds.getAll('emojis').find(emoji => emoji.name.toLowerCase() === match.replace(/:/g, '').toLowerCase())
    log.trace('emoji', emoji)
    if (emoji) {
