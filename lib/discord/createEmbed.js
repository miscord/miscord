...
const getFilename = (url, isSafeImageURL) => (isSafeImageURL && !require('url').parse(url, true).pathname.includes('.')) ? 'image.png' : require('url').parse(url, true).pathname.split('/').pop('-1')
...
module.exports = opts => {
  ...
  if (message.attachments[0].type === 'share') {
    if (message.attachments[0].image) {
      ...
      return [{
        embed: embed.setURL(message.attachments[0].url),
        files: [{
          name: getFilename(imageURL, imageURL.pathname === '/safe_image.php'),
          attachment: imageURL.pathname === '/safe_image.php' ? imageURL.query.url : message.attachments[0].image
        }]
      }]
    }
