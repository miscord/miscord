...
It's not an error... unless you added the bot to your guild already.`)
    ...
    log.trace('guilds', toStr(discord.guilds, 1))

    // if user put category name in the config
    if (config.discord.category) {
      // get category
      var category = discord.guilds[0].channels.find(channel =>
      ...
      log.trace('category', toStr(category))
    ...
    log.trace('webhooks', toStr(discord.webhooks, 1))
    ...
    if (config.errorChannel) {
      if (client.users.has(config.errorChannel)) {
        ...
        log.warn(`Error channel/user ${config.errorChannel} not found.`)
      }
    }
    ...
    if (config.commandChannel) {
      if (client.users.has(config.commandChannel)) {
        ...
        log.warn(`Command channel/user ${config.commandChannel} not found.`)
      }
    }
