...
const commands = {
  ...
  'get': argv => {
    ...
    if (['discord.token', 'messenger.username', 'messenger.password'].includes(key)) return config.commandChannel.send(`\`config.${key}\` is set to \`***\``)
  ...
  'showConfig': argv => {
    ...
    config.commandChannel.send(cleanConfig, { code: true })
  },
  'help': argv => {
