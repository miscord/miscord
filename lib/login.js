  ...
  return new Promise((resolve, reject) => {
    ...
    Messenger(obj, options, (err, api) => {
      if (err) {
        ...
        getChannel(guild, 'login-approval', category).then(channel => channel.send('Enter code:'))
...
module.exports = async () => {
  ...
  return discord.login(config.discord.token).then(async u => {
    ...
    var category
    category = guild.channels.find(channel => channel.name === 'messenger' && channel instanceof Discord.CategoryChannel)
