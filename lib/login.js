...
function getAPI (config) {
	return new Promise((resolve, reject) => {
		...
		Messenger(obj, (err, api) => {
			if (err) {
				if (err.error !== 'login-approval') reject(err)
				getChannel('login-approval', 'channel created just to get a login approval code').then(channel => channel.send('Enter code:'))
				discord.on('message', message => {
					if(loggedin || message.channel.name !== 'login-approval') return
