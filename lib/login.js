  ...
  return new Promise((resolve, reject) => {
    ...
    Messenger(obj, options, (err, api) => {
      if (err) {
        ...
        getChannel(guild, 'login-approval', category, 'channel made for codes from login approval').then(channel => channel.send('Enter code:'))
...
module.exports = async () => {
  ...
  return discord.login(config.discord.token).then(async u => {
    ...
    var category
    category = guild.channels.find(channel => channel.name === 'messenger' && channel.type === null)
    if (!category) category = await guild.createChannel('messenger', 'category')

    return getAPI(config, guild, category).then(facebook => {
      return {facebook, discord, guild, category}
    })
  })
}
