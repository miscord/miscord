...
function getAPI (config, guild, category) {
  return new Promise((resolve, reject) => {
    ...
    Messenger(obj, options, (err, api) => {
      if (err) {
        ...
        }).then(channel => channel.send('Enter code:'))
...
module.exports = async () => {
  ...
  return discord.login(config.discord.token).then(async u => {
    ...
    // find or create a category for bot channels
    var categoryName = config.discord.category ? config.discord.category : 'messenger'
    var category
    category = guild.channels.find(channel => channel.name === categoryName && channel.type === null)
    if (!category) category = await guild.createChannel(categoryName, 'category')

    return getAPI(config, guild, category).then(facebook => {
      return {facebook, discord, guild, category, config}
    })
  })
}
