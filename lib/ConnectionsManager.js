    ...
    try {
      ...
      await Promise.all(Object.entries(parsed).map(async ([name, endpoints]) => {
        ...
        endpoints = await Promise.all(endpoints.map(async endpoint => {
          ...
          if (typeof endpoint.id !== 'string') throw new Error(`Endpoint ID is not a string (wrap it in single quotes): ${endpoint.id}`)

          if (endpoint.type === 'discord' && !discord.client.channels.get(endpoint.id)) throw new Error(`Channel ${endpoint.id} not found!`)
          if (endpoint.type === 'messenger' && !(await getThread(endpoint.id))) throw new Error(`Thread ${endpoint.id} not found!`)
