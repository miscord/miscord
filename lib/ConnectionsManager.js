...
class ConnectionsManager {
  ...
  save () {
    ...
    for (const [messenger, discord] of this.channels.entries()) {
      if (typeof discord === 'string') obj[discord] = messenger
  ...
  findChannel (nameOrID) {
    return discord.client.channels.find(channel => channel.name === nameOrID || channel.id === nameOrID) || log.warn('CM', 'Discord channel %s has not been mapped properly', nameOrID) || nameOrID
  }
